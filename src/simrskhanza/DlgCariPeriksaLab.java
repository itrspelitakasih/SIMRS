...
public class DlgCariPeriksaLab extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        ppKirimPasien = new javax.swing.JMenuItem();
        ppKirimDokter = new javax.swing.JMenuItem();
        ...
        ppKirimPasien.setBackground(new java.awt.Color(255, 255, 254));
        ppKirimPasien.setFont(new java.awt.Font("Tahoma", 0, 11)); // NOI18N
        ppKirimPasien.setForeground(new java.awt.Color(50, 50, 50));
        ppKirimPasien.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/category.png"))); // NOI18N
        ppKirimPasien.setText("Kirim ke WA Pasien");
        ppKirimPasien.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        ppKirimPasien.setHorizontalTextPosition(javax.swing.SwingConstants.RIGHT);
        ppKirimPasien.setName("ppKirimPasien"); // NOI18N
        ppKirimPasien.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                ppKirimPasienActionPerformed(evt);
            }
        });
        jPopupMenu1.add(ppKirimPasien);

        ppKirimDokter.setBackground(new java.awt.Color(255, 255, 254));
        ppKirimDokter.setFont(new java.awt.Font("Tahoma", 0, 11)); // NOI18N
        ppKirimDokter.setForeground(new java.awt.Color(50, 50, 50));
        ppKirimDokter.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/category.png"))); // NOI18N
        ppKirimDokter.setText("Kirim ke WA Dokter");
        ppKirimDokter.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        ppKirimDokter.setHorizontalTextPosition(javax.swing.SwingConstants.RIGHT);
        ppKirimDokter.setName("ppKirimDokter"); // NOI18N
        ppKirimDokter.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                ppKirimDokterActionPerformed(evt);
            }
        });
        jPopupMenu1.add(ppKirimDokter);
...
private void tbDokterKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tbDokterKeyPressed
   if(tabMode.getRowCount()!=0){
    ...
    private void ppKirimPasienActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppKirimPasienActionPerformed
        this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
        ...
        try {
            ...
            while (rs.next()) {
                ...
                try {
                    ...
                    String ownerPass = "AmpunBangJago"; // ganti kebijakan internal
                    ...
                    String fileUrl = "https://waha.rrpelitakasih.id/HasilLab/" + enc.getName();
                    ...
                    if (noHP.equals("")) {
                        ...
                        + "Contoh: 16012021\n\n"
                        + "Jika ada pertanyaan, silakan hubungi Care Center kami via WhatsApp di nomor .\n"
                        + "Terima kasih 🙏";
                    ...
                    String fileUrl = "https://waha.rrpelitakasih.id/HasilLab/" + out.getName();
                    String noHP = Sequel.cariIsi("select no_tlp from pasien where no_rkm_medis=?", rs.getString("no_rkm_medis")).replaceAll("[^0-9]", "");
                    if (!noHP.equals("")) {
    ...
    private void ppKirimDokterActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppKirimDokterActionPerformed
        this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
        System.out.println("[WAHA] Kirim Hasil Lab ke Dokter Perujuk dimulai...");
        ...
        try {
            ps4 = koneksi.prepareStatement(
                ...
                + "periksa_lab.dokter_perujuk, dperujuk.nm_dokter AS nama_dokter_perujuk, "
                + "DATE_FORMAT(periksa_lab.tgl_periksa,'%d-%m-%Y') AS tgl_periksa, periksa_lab.jam "
                + "FROM periksa_lab "
                + "INNER JOIN reg_periksa ON periksa_lab.no_rawat = reg_periksa.no_rawat "
                + "INNER JOIN pasien ON reg_periksa.no_rkm_medis = pasien.no_rkm_medis "
                + "LEFT JOIN dokter dperujuk ON dperujuk.kd_dokter = periksa_lab.dokter_perujuk "
                + "WHERE periksa_lab.kategori = 'PK' AND periksa_lab.tgl_periksa = ? "
                + "AND periksa_lab.jam = ? AND periksa_lab.no_rawat = ? "
            ...
            while (rs.next()) {
                ...
                String tgl = Valid.SetTgl(rs.getString("tgl_periksa")).replaceAll("-", "");
                String jam = rs.getString("jam").replaceAll(":", "");
                String safeNoRawat = noRawat.replaceAll("[^0-9]", "");
                String namaFile = safeNoRawat + "_" + tgl + "_" + jam + "_" + noRM + ".pdf";

                File pdfFile = new File("./HasilLab/" + namaFile);
                if (!pdfFile.exists() || pdfFile.length() == 0) {
                    JOptionPane.showMessageDialog(null, "❌ File PDF belum tersedia untuk dikirim ke dokter perujuk.");
                    continue;
                }

                String urlPublik = WAKirim.uploadPDFToServer(pdfFile);
                if (urlPublik == null || !urlPublik.contains("http")) {
                    JOptionPane.showMessageDialog(null, "❌ Upload file gagal untuk: " + namaFile);
                    continue;
                }

                String noHPDokter = Sequel.cariIsi(
                    "SELECT no_telp FROM dokter WHERE kd_dokter = ?",
                    rs.getString("dokter_perujuk")
                ).replaceAll("[^0-9]", "");
                String nomorWA = WAKirim.toPhoneOnly(noHPDokter);

                if (nomorWA.isEmpty()) {
                    JOptionPane.showMessageDialog(null, "❌ Nomor HP dokter tidak valid atau kosong.");
                    continue;
                }

                String pesan = "Yth. " + rs.getString("nama_dokter_perujuk") + ",\n"
                + "Berikut hasil lab pasien *" + namaPasien + "* dari " + akses.getnamars() + ":\n"
                + urlPublik;

                WAKirim.SendResult result = WAKirim.kirimTeksWithStatus(nomorWA, pesan);

                if (result.ok) {
                    System.out.println("[WAHA] WA ke dokter berhasil.");
                    JOptionPane.showMessageDialog(null, "✅ Hasil lab berhasil dikirim ke dokter perujuk.");
                } else {
                    System.out.println("[WAHA] Gagal kirim WA ke dokter → " + result.responseBody);

                    if (result.responseBody != null && result.responseBody.contains("\"status\":500")) {
                        JOptionPane.showMessageDialog(null, "⚠️ Pesan mungkin sudah terkirim meskipun ada error:\n" + result.responseBody);
                    } else {
                        JOptionPane.showMessageDialog(null, "❌ Gagal mengirim WA ke dokter.\n" + result.responseBody);
                    }
                }
            }

        } catch (Exception e) {
            JOptionPane.showMessageDialog(null, "❌ Error kirim ke dokter perujuk: " + e.getMessage());
            e.printStackTrace();
        } finally {
            try {
                if (rs != null) {
            ...
            this.setCursor(Cursor.getDefaultCursor());
            System.out.println("[WAHA] Selesai kirim ke dokter perujuk");
        }
    }//GEN-LAST:event_ppKirimDokterActionPerformed
    ...
    */
    ...
    private javax.swing.JMenuItem ppKirimDokter;
    private javax.swing.JMenuItem ppKirimPasien;
